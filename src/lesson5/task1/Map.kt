...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((name, grade) in grades) {
        val tmp = res[grade]
        if (tmp == null) res[grade] = mutableListOf(name)
        else tmp.add(name)
    }
    return res
}
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    for ((org, price) in stockPrices) {
        if (!newMap.containsKey(org)) {
            ...
            k[org] = k.getValue(org) + 1
        }
    }
    for ((org, price) in newMap) {
        newMap[org] = price / k[org]!!
    }
    return newMap
}
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    val necessary = mutableSetOf<Char>()
    for (each in chars) {
        necessary.add(each.toLowerCase())
    }
    for (char in word.toSet()) {
        if (char.toLowerCase() !in necessary) return false
    }
    return true
}
...
fun propagateHandshakes(friends: Map<String, Set<String>>): Map<String, Set<String>> {
    ...
    for ((name, mates) in friends) {
        val newMates = mutableSetOf<String>()
        fun helper(mate: Set<String>): Set<String> {
            for (one in mate) {
                if (friends.containsKey(one)) {
                    for (each in friends[one] ?: error("")) {
                        if (each !in mates + newMates && each != name) {
                            newMates.add(each)
                            helper(setOf(each))
                        }
                    }
        ...
        if (mates.isNotEmpty()) res[name] = helper(mates) + mates
        else res[name] = setOf()
    }
return res
}
